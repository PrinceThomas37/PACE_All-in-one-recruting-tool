...
| `R-038` | **Let AI write the cold emails the leads engine sends** — one draft per lead, written from that lead's own posting, through the same checker the Generator uses; the templates stay as the fallback. | Suggested 2026-09-23 after the owner asked whether the engine's emails are AI-written. **They are not:** measured that day, 119 queued emails, all filled-in templates (56 from 5 rotating variants, 63 from the single default). AI writes only in Email → Compose → Generator. Cost to weigh: an AI call per lead, against a free daily AI allowance shared with resume parsing and the JD scrub. |
