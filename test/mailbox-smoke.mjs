...
// ════════════════════════════════════════════════════════════════════════════
{
  ...
  const MAILBOXES = {
    'mb-mine':        { id: 'mb-mine', user_id: ME, org_id: ORG, email_address: 'me@pace.io', platform: 'Microsoft', is_active: true },
  ...
  app.use(require('../routes/mailbox.js')({
    ...
    graphMailRequest: async () => { providerCalls++; return { value: [] }; },
    getMicrosoftToken: async () => { providerCalls++; return 'tok'; },
    gmailProvider: { isConfigured: () => true },
    sendMicrosoftNewMessage: async () => { providerCalls++; return {}; },
    buildHtmlEmailBody: (t) => t,
